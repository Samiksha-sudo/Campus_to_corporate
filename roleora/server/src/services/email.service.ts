...
export async function sendVerificationEmail(to: string, firstName: string, token: string): Promise<void> {
  ...
  await send(to, 'Verify your Roleora email', `
    <p>Hi ${firstName},</p>
    <p>Please verify your email address by clicking the link below:</p>
    <p><a href="${link}">Verify email</a></p>
    <p>This link expires in 24 hours.</p>
    <p>If you didn't create a Roleora account, you can ignore this email.</p>
  `)
}

export async function sendPasswordResetEmail(to: string, firstName: string, token: string): Promise<void> {
  const link = `${env.APP_URL}/reset-password?token=${token}`
  await send(to, 'Reset your Roleora password', `
    <p>Hi ${firstName},</p>
...
export async function sendWelcomeEmail(to: string, firstName: string): Promise<void> {
  await send(to, 'Welcome to Roleora', `
    <p>Hi ${firstName},</p>
    <p>Welcome to Roleora — your career, managed.</p>
